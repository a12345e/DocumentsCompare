...
class TheDocAnalyzer(DocAnalyzer):
    ...
    def get_field_type(self, field_name: string) -> FieldType:
        if field_name.startswith(FieldType.KEY_IGNORE.name):
            return FieldType.KEY_IGNORE
        if field_name.startswith(FieldType.VALUE_IGNORE.name):
            return FieldType.VALUE_IGNORE
        if field_name.startswith(FieldType.VALUE_RELEVANT.name):
...
class TestDocAnalyzer(unittest.TestCase):
    ...
    def test_the_analyzer_classifier(self):
        ...
        self.assertEqual(analyzer.get_doc_classifier({'a': 'a'}), 'unclassified')
        self.assertEqual(analyzer.get_doc_classifier({'classifier': 'a', 'b': 'a'}), 'a')

    def test_the_analyzer_field_typing(self):
        analyzer = TheDocAnalyzer()
        self.assertEqual(analyzer.get_field_type('a'), FieldType.PART_OF_KEY)
        self.assertEqual(analyzer.get_field_type('PART_OF_KEY_x'), FieldType.PART_OF_KEY)
        self.assertEqual(analyzer.get_field_type('KEY_IGNORE'), FieldType.KEY_IGNORE)
        self.assertEqual(analyzer.get_field_type('VALUE_IGNORE_y'), FieldType.VALUE_IGNORE)
        self.assertEqual(analyzer.get_field_type('VALUE_RELEVANT_u'), FieldType.VALUE_RELEVANT)
    ...
    def test_analyze_document(self):
        ...
        self.assertEqual(set(analyzed_document.get_key_lists()[FieldType.KEY_IGNORE]),
                         set(['KEY_IGNORE_y', 'KEY_IGNORE_x']))
        self.assertEqual(set(analyzed_document.get_key_lists()[FieldType.PART_OF_KEY]),
                         set(['PART_OF_KEY_y', 'PART_OF_KEY_x', 'a', 'classifier']))
        self.assertEqual(set(analyzed_document.get_key_lists()[FieldType.VALUE_IGNORE]),
                         set(['VALUE_IGNORE_x', 'VALUE_IGNORE_y']))
        self.assertEqual(set(analyzed_document.get_key_lists()[FieldType.VALUE_RELEVANT]),
